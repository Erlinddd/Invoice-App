...
function Faturat(props) {
  ...
  useEffect(() => {
    const GetData = async () => {
      const result = await axiosInstance.get(`/fatura`);
      setData(result.data);
  ...
  const getById = async (event) => {
    ...
    const response = await axiosInstance.get(`/fatura/` + IdFatura);
  ...
  const deleteFatura = (idFatura) => {
    axiosInstance.delete("/fatura/" + idFatura).then((result) => {
  ...
  const onsubmit = async (e) => {
    e.preventDefault();
    const response = await axiosInstance.get("/fatura/getbydates");
          ...
          <div className="card text-center" style={marginBottom}>
            ...
            <div className="card-body">
              ...
              <p className="card-text">
                <h3>
                  Koha e faktures: {moment(dat.data).format("YYYY-MM-DD hh:mm")}
                </h3>{" "}
                <h5>Bleresi: {dat?.bleresi?.firstName}</h5>
              </p>
              {/* <Link>
