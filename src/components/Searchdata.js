...
export class Searchdata extends Component {
  ...
  onsubmit = (e) => {
    ...
    axiosInstance
      .get(`/fatura/GetByDates${"?from=" + startDate + "&to=" + endDate}`)
      .then((response) => {
            ...
            {this.state.faturadate.length === 0 ? (
              <tr align="center">
                <td colSpan="6" className="text-white">
                  {" "}
                  There are no invoice to show between selected date{" "}
                </td>
              </tr>
            ) : (
                ...
                return (
                  <tr key={index}>
                    ...
                    <td className="text-white">
                      {moment(p.data).format("YYYY-MM-DD hh:mm")}
                    </td>
                    <td className="text-white">
                      <Link
