...
class ArticleListDDL extends Component {
  ...
  faturaList = (props) => {
    return this.props.history.push(`/faturat`);
  };
  ...
  submitData = () => {
    ...
    axios
      .post("https://localhost:44362/api/fatura", myState)
      .then((response) => {
  ...
  async getConsumer(e) {
    axiosInstance.get(`/bleresi`).then((res) => {
  ...
  getTotal() {
    ...
    // }
    var total = this.state.selectedItems.reduce(function (
      accumulator,
      currentValue
    ) {
  ...
  render() {
    const { items, selectedItems, consumer, selectedConsumer, data } =
      this.state;
    return (
      <div>
        {this.state.loading ? (
              ...
              <div>
                <div style={{ display: this.state.show ? "block" : "none" }}>
                  <MyToast
                    ...
                    message={"Fatura u regjistrua me sukses"}
                    type={"success"}
                  />
                </div>
                <Alert variant="success">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      this.setState({
                        selectedConsumer: parseInt(e.target.value),
                      });
                    }}
                  >
                    <option value="">Select from the list</option>
                    {this.state.consumer.map((person, id) => (
                  ...
                  <Select
                    ...
                    value={selectedItems}
                    placeholder="Select article from the list... "
                  ...
                  <p style={{ marginLeft: "820px" }}>
                    Vlera totale: {this.state.totalInvoiceValue}
                  </p>
                </Alert>
                <Button
